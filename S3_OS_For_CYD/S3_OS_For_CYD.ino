...
#include <functional>
#include "driver/gpio.h" // Required for pin holding
  ...
  while (inMenu) {
    ...
    if (touch.touched()) {
      ...
      if (tappedIdx >= 0 && tappedIdx <= 4) {
        ...
        else { sel = tappedIdx; redraw = true; tone(BUZZER_PIN, 800, 5); }
      }
      delay(150); 
    }
    ...
    if (kbdBack) { kbdBack = false; inMenu = false; }
    vTaskDelay(2); 
  }
}
...
void drawFileLine(int index, bool highlight) {
  ...
  for (int i = 0; i < skipCount; i++) { File e = root.openNextFile(); e.close(); }
  File entry = root.openNextFile();
  if (!entry) { root.close(); return; }
  gfx->fillRect(0, y, 320, 24, highlight ? 0x07E0 : 0x0000); 
  gfx->setCursor(10, y + 4);
...
void fullRedraw() {
  gfx->fillScreen(0x0000);
  gfx->setTextColor(0x07E0); gfx->setTextSize(2);
  gfx->setCursor(10, 5); gfx->print("PATH: "); gfx->println(currentPath);
  gfx->setCursor(10, 25); gfx->setTextColor(0xFFFF); gfx->setTextSize(1);
  gfx->println("N:New M:MkDir D:Del R:Ren S+ENT:Open"); 
  gfx->drawFastHLine(0, 40, 320, 0x07E0); 
  fileCount = (currentPath == "/") ? 1 : 0; 
  File root = SD.open(currentPath);
  while (File entry = root.openNextFile()) { fileCount++; entry.close(); }
  root.close();
  for (int i = 0; i < 8; i++) {
...
void playMJPEG(String path) {
  ...
  if (!mjpeg_buf) { mjpegFile.close(); return; }
  gfx->fillScreen(0x0000);
  ...
  while (mjpegFile.available() && !kbdBack) {
    ...
    while (mjpegFile.available()) {
      if (mjpegFile.read() == 0xFF) {
        if (mjpegFile.peek() == 0xD8) {
          mjpeg_buf[0] = 0xFF; mjpeg_buf[1] = mjpegFile.read();
    ...
    while (mjpegFile.available() && idx < buf_size - 1) {
      ...
      mjpeg_buf[idx++] = b;
      if (b == 0xFF && mjpegFile.peek() == 0xD9) { mjpeg_buf[idx++] = mjpegFile.read(); break; }
    }
  ...
  free(mjpeg_buf); mjpegFile.close(); kbdBack = false; fullRedraw();
}
...
String inputPrompt(String title) {
  ...
  gfx->setCursor(30, 90); gfx->setTextColor(0xFFFF); gfx->setTextSize(2); gfx->print(title);
  while (true) {
    checkPowerButton();
    gfx->fillRect(30, 120, 260, 30, 0x0000);
    gfx->setCursor(35, 125); gfx->print(input + "_");
    if (lastRawKey != 0) {
      uint8_t rk = lastRawKey; char c = lastKey;
      lastRawKey = 0; lastKey = 0;
...
static int l_getTouch(lua_State *L) {
    if (touch.touched()) {
        ...
        lua_pushboolean(L, true); lua_pushinteger(L, x); lua_pushinteger(L, y);
        return 3;
    }
    lua_pushboolean(L, false); return 1;
}
static int l_setTextSize(lua_State *L) { gfx->setTextSize((int)luaL_optinteger(L, 1, 1)); return 0; }
static int l_cls(lua_State *L) { gfx->fillScreen((uint16_t)luaL_optinteger(L, 1, 0x0000)); return 0; }
...
static int l_rect(lua_State *L) { gfx->drawRect(luaL_checkinteger(L, 1), luaL_checkinteger(L, 2), luaL_checkinteger(L, 3), luaL_checkinteger(L, 4), (uint16_t)luaL_checkinteger(L, 5)); return 0; }
static int l_fillRect(lua_State *L) { gfx->fillRect(luaL_checkinteger(L, 1), luaL_checkinteger(L, 2), luaL_checkinteger(L, 3), luaL_checkinteger(L, 4), (uint16_t)luaL_checkinteger(L, 5)); return 0; }
...
static int l_playSound(lua_State *L) { tone(BUZZER_PIN, luaL_checkinteger(L, 1), luaL_checkinteger(L, 2)); return 0; }
static int l_getJoystick(lua_State *L) {
    ...
    lua_pushinteger(L, x); lua_pushinteger(L, y); lua_pushboolean(L, kbdEnter);
    return 3;
}
...
static int l_drawImg(lua_State *L) {
    ...
    int x = luaL_checkinteger(L, 2); int y = luaL_checkinteger(L, 3);
    int scale = (int)luaL_optinteger(L, 4, 1);
    String fullPath = currentPath + (currentPath.endsWith("/") ? "" : "/") + String(path);
    TJpgDec.setJpgScale(scale); TJpgDec.drawSdJpg(x, y, fullPath.c_str());
    return 0;
}
...
void runLua(String filename) {
    ...
    luaL_openlibs(L);
    lua_register(L, "getTouch", l_getTouch); lua_register(L, "setTextSize", l_setTextSize);
    lua_register(L, "cls", l_cls); lua_register(L, "printAt", l_printAt);
    lua_register(L, "isKeyDown", l_isKeyDown); lua_register(L, "rect", l_rect);
    lua_register(L, "fillRect", l_fillRect); lua_register(L, "circle", l_circle);
    lua_register(L, "playSound", l_playSound); lua_register(L, "getJoystick", l_getJoystick);
    lua_register(L, "delay", l_delay); lua_register(L, "drawImg", l_drawImg);
    gfx->setTextSize(1); gfx->fillScreen(0x0000);
    if (luaL_dostring(L, code.c_str()) != 0) {
        gfx->setTextColor(0xF800); gfx->setCursor(10, 10);
        gfx->println("LUA ERROR:"); gfx->println(lua_tostring(L, -1));
        while (!kbdBack) { checkPowerButton(); delay(10); }
    } else {
        while (!kbdBack) { checkPowerButton(); delay(10); }
    }
    lua_close(L); kbdBack = false;
}
...
char hidToAscii(uint8_t key, uint8_t mod) {
  ...
  if (key >= 0x1E && key <= 0x27) {
    if (!shift) { if (key == 0x27) return '0'; return '1' + (key - 0x1E); }
    else { const char shiftNumbers[] = "!@#$%^&*()"; return shiftNumbers[key - 0x1E]; }
  }
  if (key == 0x2C) return ' '; if (key == 0x28) return '\n'; 
  if (key == 0x2D) return (shift ? '_' : '-'); if (key == 0x2E) return (shift ? '+' : '=');
  if (key == 0x33) return (shift ? ':' : ';'); if (key == 0x34) return (shift ? '"' : '\'');
  if (key == 0x36) return (shift ? '<' : ','); if (key == 0x37) return (shift ? '>' : '.');
...
static void notifyCallback(NimBLERemoteCharacteristic* pChar, uint8_t* pData, size_t length, bool isNotify) {
  ...
  for (int i = 2; i < length; i++) {
    if (pData[i] != 0) { pressedKeys.insert(pData[i]); lastRawKey = pData[i]; hasKey = true; }
  }
  if (!hasKey) { kbdY = 0; kbdEnter = false; kbdBack = false; lastRawKey = 0; }
  else {
...
void runEditor(String filename) {
  inEditor = true; editorBuffer = ""; int bufferIndex = 0; 
  int scrollY = 0; 
  String fullPath = currentPath + (currentPath.endsWith("/") ? "" : "/") + filename;
  File f = SD.open(fullPath, FILE_READ);
  if (f) { while(f.available()) editorBuffer += (char)f.read(); f.close(); }
  bufferIndex = editorBuffer.length();

  auto drawHeader = [&]() {
    gfx->fillRect(0, 0, 320, 33, 0x0000); 
    gfx->setCursor(0, 0); gfx->setTextColor(0x07E0); gfx->printf("EDIT: %s", filename.c_str());
    gfx->setCursor(0, 18); gfx->println("[ ] Move | TAB Save | ESC Exit");
    gfx->drawFastHLine(0, 32, 320, 0x07E0);
  };

  gfx->fillScreen(0x0000);
  drawHeader();
  gfx->setCursor(0, 35 - scrollY); gfx->setTextColor(0xFFFF); gfx->print(editorBuffer);

  while(inEditor) {
    checkPowerButton();
    static unsigned long lastFlash = 0; static bool cursorOn = true;
    ...
    if (curY > 220 || (curY < 35 && scrollY > 0)) {
      ...
      drawHeader(); gfx->setCursor(0, 35 - scrollY); gfx->setTextColor(0xFFFF); gfx->print(editorBuffer);
    }

    if (millis() - lastFlash > 400) {
      lastFlash = millis(); cursorOn = !cursorOn;
      gfx->setCursor(0, 35 - scrollY); 
      gfx->print(editorBuffer.substring(0, bufferIndex));
    ...
    if (lastRawKey != 0) {
      uint8_t rk = lastRawKey; char c = lastKey;
      gfx->setCursor(0, 35 - scrollY); gfx->print(editorBuffer.substring(0, bufferIndex));
      int16_t oldX = gfx->getCursorX(); int16_t oldY = gfx->getCursorY();

      if (rk == 0x2A) { 
        if (bufferIndex > 0) {
          gfx->fillRect(oldX, oldY, 4, 16, 0x0000);
          editorBuffer = editorBuffer.substring(0, bufferIndex - 1) + editorBuffer.substring(bufferIndex);
          bufferIndex--; gfx->fillRect(0, 33, 320, 207, 0x0000);
          gfx->setCursor(0, 35 - scrollY); gfx->print(editorBuffer);
        }
        lastRawKey = 0;
      }
      else if (rk == 0x4F && bufferIndex < editorBuffer.length()) { gfx->fillRect(oldX, oldY, 4, 16, 0x0000); bufferIndex++; lastRawKey = 0; }
      else if (rk == 0x50 && bufferIndex > 0) { gfx->fillRect(oldX, oldY, 4, 16, 0x0000); bufferIndex--; lastRawKey = 0; }
      else if (c != 0 || rk == 0x28) { 
        char toAdd = (rk == 0x28) ? '\n' : c;
        gfx->fillRect(oldX, oldY, 4, 16, 0x0000);
        editorBuffer = editorBuffer.substring(0, bufferIndex) + toAdd + editorBuffer.substring(bufferIndex);
        bufferIndex++; 
        if (rk == 0x28) { gfx->fillRect(0, 33, 320, 207, 0x0000); gfx->setCursor(0, 35 - scrollY); gfx->print(editorBuffer); }
        else { gfx->setCursor(oldX, oldY); gfx->print(editorBuffer.substring(bufferIndex - 1)); }
        lastRawKey = 0; lastKey = 0; 
      }
      else if (rk == 0x29) { inEditor = false; lastRawKey = 0; }
      else if (rk == 0x2B) {
        SD.remove(fullPath); File saveF = SD.open(fullPath, FILE_WRITE);
        saveF.print(editorBuffer); saveF.close(); lastRawKey = 0;
      }
    }
    vTaskDelay(5);
  }
...
void setup() {
  // Release any pin holds from previous sleep cycle
  gpio_hold_dis((gpio_num_t)LED_RED);
  gpio_hold_dis((gpio_num_t)LED_GREEN);
  gpio_deep_sleep_hold_dis();

  // Initialize LEDs (HIGH = Off for Common Anode)
  pinMode(LED_RED, OUTPUT);
  pinMode(LED_GREEN, OUTPUT);
  pinMode(LED_BLUE, OUTPUT);
  digitalWrite(LED_RED, HIGH);
  digitalWrite(LED_BLUE, HIGH);
  digitalWrite(LED_GREEN, LOW); // Turn Green ON for "Awake"
...
void loop() {
  ...
  if (touch.touched()) {
    TS_Point p = touch.getPoint();
    int ty = map(p.y, 240, 3800, 0, 240);
    if (lastTouchY == -1) { lastTouchY = ty; touchStartTime = millis(); isSwiping = false; }
    else {
      int dy = ty - lastTouchY;
      if (dy > 20) { kbdY = -1; lastTouchY = ty; isSwiping = true; }
      else if (dy < -20) { kbdY = 1; lastTouchY = ty; isSwiping = true; }
    }
  } else {
    if (lastTouchY != -1) {
      if (!isSwiping && (millis() - touchStartTime) < 300) { 
        if (lastTouchY > 45) { 
          ...
          if (idx >= 0 && idx < 8 && (idx + displayOffset) < fileCount) {
            ...
            else { cursor = idx + displayOffset; fullRedraw(); }
          }
        }
      }
      lastTouchY = -1;
  ...
  if (kbdY != 0) {
    ...
    if (cursor < displayOffset) { displayOffset = cursor; fullRedraw(); }
    else if (cursor >= displayOffset + 8) { displayOffset = cursor - 7; fullRedraw(); }
  ...
  if (lastRawKey == 0x11) { 
    lastRawKey = 0; String name = getNewFileName();
    File f = SD.open(currentPath + (currentPath.endsWith("/")?"":"/") + name, FILE_WRITE);
    f.close(); runEditor(name);
  }
  if (lastRawKey == 0x10) { 
    lastRawKey = 0; String folderName = inputPrompt("Folder Name:");
    if (folderName != "") { SD.mkdir(currentPath + (currentPath.endsWith("/")?"":"/") + folderName); fullRedraw(); }
  }
  if (lastRawKey == 0x07) { 
    lastRawKey = 0; File root = SD.open(currentPath);
    int t = (currentPath == "/" && cursor > 0) ? cursor - 1 : cursor;
    for(int i=0; i<t; i++) { File e = root.openNextFile(); e.close(); }
    File entry = root.openNextFile();
    if (entry) {
      String full = currentPath + (currentPath.endsWith("/")?"":"/") + entry.name();
      entry.close(); if (!SD.remove(full)) SD.rmdir(full);
      cursor = 0; displayOffset = 0; fullRedraw();
    }
    root.close();
  }
  if (lastRawKey == 0x15) { 
    ...
    File entry = root.openNextFile();
    if (entry) {
      String oldName = entry.name(); entry.close();
  ...
  if (kbdBack && currentPath != "/") {
    int lastSlash = currentPath.lastIndexOf('/', currentPath.length() - 2);
    currentPath = currentPath.substring(0, lastSlash + 1);
    cursor = 0; displayOffset = 0; kbdBack = false; fullRedraw(); delay(300);
  }

  if (kbdEnter) {
    bool useEditor = kbdShift; kbdEnter = false;
    if (currentPath == "/" && cursor == 0) { gameLauncherMenu(); fullRedraw(); }
    else {
      File root = SD.open(currentPath);
      int target = (currentPath == "/") ? cursor - 1 : cursor;
      for(int i = 0; i < target; i++) { File e = root.openNextFile(); e.close(); }
      File entry = root.openNextFile();
      if (entry) {
        String name = entry.name();
        String fullFileName = currentPath + (currentPath.endsWith("/") ? "" : "/") + name;
        if (entry.isDirectory()) { currentPath += (currentPath.endsWith("/") ? "" : "/") + name; cursor = 0; displayOffset = 0; fullRedraw(); }
        else if (name.endsWith(".lua")) { if (useEditor) runEditor(name); else runLua(name); fullRedraw(); }
        else if (name.endsWith(".txt")) runEditor(name);
        else if (name.endsWith(".jpg") || name.endsWith(".jpeg")) {
          gfx->fillScreen(0x0000); TJpgDec.setJpgScale(1); TJpgDec.drawSdJpg(0, 0, fullFileName.c_str());
          while(!kbdBack) { checkPowerButton(); vTaskDelay(10); } kbdBack = false; fullRedraw();
        } else if (name.endsWith(".mjpeg") || name.endsWith(".mjpg")) { playMJPEG(fullFileName); fullRedraw(); }
        entry.close();
      }
      root.close();
    }
